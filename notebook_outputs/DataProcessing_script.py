...
# Process Genes Encoding Surface Proteins
...
membrane_protein_ccle = pd.read_csv("mass_spec_valid_surface_protein.csv", header=0, index_col=0)
memprot_ccle = membrane_protein_ccle["ENTREZ gene symbol"]

# Source 2: Uniprot derived membrane protein list

membrane_protein_uniprot = pd.read_csv("uniprot_subcellular_localization", sep="\t",header=0, index_col=0)
...
# --- 2: Only keep the surface genes ---
surface_gene = set(pd.read_csv('mass_spec_valid_surface_protein.csv')['ENTREZ gene symbol'])
df_clean = df_clean[df_clean['Gene name'].isin(surface_gene)]

# --- 3: Remove non-membrane genes ---
gene_pattern = re.compile(r'\d+,([A-Za-z0-9]+),')
non_membrane_genes = set(gene_pattern.findall(uniprot_non_membrane_gene))
...
# For the cleaned data picking highest nTPM from clusters
...
df_high = (
    df_clean.sort_values(["Tissue", "Cell type", "Gene name", "nTPM"], ascending=[True, True, True, False])
    .drop_duplicates(subset=["Tissue", "Cell type", "Gene name"], keep="first")
)

# Fix gene list (all unique surface genes that survived filtering)
gene_list_high = sorted(df_high["Gene name"].unique())

# Create tissue_cells (tissue-cell pairs)
tissue_cells_high = df_high[["Tissue", "Cell type"]].drop_duplicates().values.tolist()

# Build the nTPM matrix table: rows = (Tissue, Cell type), columns = Gene names, values = highest nTPM
nTPM_matrix_high_df = (
    df_high
    .pivot_table(index=["Tissue", "Cell type"], columns="Gene name", values="nTPM", fill_value=0)
    .reindex(columns=gene_list_high)  # Make sure columns are ordered by gene_list
    .reset_index()
)

# Drop the tissue/cell columns to get just the nTPM matrix
# nTPM_matrix = nTPM_matrix_df.drop(columns=["Tissue", "Cell type"]).values.tolist()

# --- Write outputs ---
# Write gene expression table
nTPM_matrix_high_df .to_csv('gene_expression_matrix_high.csv', sep='\t', index=False)
print(nTPM_matrix_high_df.shape)


# For the cleaned data picking median nTPM from clusters
...
# Fix gene list (all unique surface genes that survived filtering)
gene_list_median = sorted(df_median["Gene name"].unique())

# Create tissue_cells (tissue-cell pairs)
tissue_cells_median = df_median[["Tissue", "Cell type"]].drop_duplicates().values.tolist()

# Build the nTPM matrix table: rows = (Tissue, Cell type), columns = Gene names, values = median nTPM
nTPM_matrix_median_df = (
    df_median
    .pivot_table(index=["Tissue", "Cell type"], columns="Gene name", values="nTPM", fill_value=0)
    .reindex(columns=gene_list_median)  # Make sure columns are ordered by gene_list
    .reset_index()
)
...
# Write gene expression table
nTPM_matrix_median_df.to_csv('gene_expression_matrix_median.csv', sep='\t', index=False)


# In[ ]:


gene_list_high == gene_list_median


# In[ ]:


tissue_cells_high = tissue_cells_median
...
# Write tissue-cell pairs
tissue_cell_df = pd.DataFrame(tissue_cells, columns=["Tissue", "Cell type"])
tissue_cell_df.to_csv('tissue_cell_pairs.tsv', sep='\t', index=False)

# Write gene list
gene_list_df = pd.DataFrame(gene_list, columns=["Gene name"])
gene_list_df.to_csv('gene_list.csv', index=False, header=False)
...
Cell_marker = pd.read_excel('Original data/Cell_marker_Human.xlsx')
...
# Summarize positive label statistics
positive_cell_count = len(positives)
positive_label_count = sum([len(gene_names) for gene_names in positives.values()])

print(f"Number of cells with positive labels: {positive_cell_count}")
print(f"Total number of positive labels: {positive_label_count}")

# Write positive labels as df
positives_records = []
for (tissue, cell_type), gene_names in positives.items():
    positives_records.append({
        "Tissue": tissue,
        "Cell type": cell_type,
        "Positive Gene Names": gene_names
    })

positives_df = pd.DataFrame(positives_records)

# Save positives_df to CSV
positives_df.to_csv('positives_labels.csv', sep='\t', index=False)
...
for tissue, cell in tissue_cells:
    if tissue not in tissue_to_cells:
        tissue_to_cells[tissue] = set()  # use set to ensure uniqueness
    ...
    for other_cell_type in tissue_to_cells.get(tissue, []):
        if other_cell_type != cell_type:
            ...
            if key in negatives:
                ...
                negatives[key] = positive_genes

# Summarize negative label statistics=
negative_cell_count = len(negatives)
negative_label_count = sum([len(gene_names) for gene_names in negatives.values()])

print(f"Number of cells with negative labels: {negative_cell_count}")
print(f"Total number of negative labels: {negative_label_count}")

# Convert negatives dictionary to DataFrame
negatives_records = []
for (tissue, cell_type), gene_names in negatives.items():
    negatives_records.append({
        "Tissue": tissue,
        "Cell type": cell_type,
        "Negative Gene Names": gene_names
    })

negatives_df = pd.DataFrame(negatives_records)

# Save negatives_df to CSV
negatives_df.to_csv('negative_labels.csv', sep='\t', index=False)
